...
def main():
    ...
    check('marketing_experiment_evidence' in migrations and 'wilson_low_ppm' in migrations and 'sufficient_sample' in migrations,'experiment evidence schema stores interval/sample evidence')
    check('commerce_abandonment_cases' in migrations and 'fk_abandonment_attempt' in migrations and 'ix_abandonment_state' in migrations,'guarded abandonment queue is migrated')
    check('marketing_growth_decisions' in migrations and 'authoritative_revenue_minor' in migrations and 'uq_growth_decision_key' in migrations,'closed-loop growth evidence is migrated')
    cmake=(ROOT/'core/CMakeLists.txt').read_text()
    check('src/internal.c' in cmake and 'src/telemetry.c' in cmake and 'src/payment_recovery.c' in cmake and 'src/search_science.c' in cmake and 'src/experiment_science.c' in cmake and 'src/abandonment.c' in cmake and 'src/growth_loop.c' in cmake and 'src/marketing_runtime.c' in cmake,'commerce telemetry/recovery/search/experiment/abandonment/growth and marketing runtime are modular')
    core_src=(ROOT/'core/src/core.c').read_text()
    marketing_runtime=(ROOT/'core/src/marketing_runtime.c').read_text()
    growth_loop=(ROOT/'core/src/growth_loop.c').read_text()
    check('fvs_marketing_job_claim' not in core_src and 'fvs_marketing_job_claim' in marketing_runtime,'marketing claimant is extracted from core hotspot')
    check('"google"' in core_src and '"email"' in core_src and '"webhook"' in core_src,'cart commercial-origin contract covers paid/search/email/webhook attribution')
    ...
    check('experiment_snapshot' in shim_core and 'experimentSnapshot' in (ROOT/'shim-php/Core.php').read_text(),'both shims expose experiment evidence snapshots')
    check('abandonment_scan' in shim_core and 'abandonmentScan' in (ROOT/'shim-php/Core.php').read_text(),'both shims expose guarded abandonment scanning')
    check('growth_loop_run' in shim_core and 'growthLoopRun' in (ROOT/'shim-php/Core.php').read_text() and 'growth_decision_apply' in shim_core,'both shims expose guarded closed-loop growth')
    attribution=(ROOT/'frontend/js/attribution.js').read_text()
    check("campaign_id:" in attribution and "creative_id:" in attribution,'browser exposure attribution propagates campaign/creative ids')
    ui_runtime=(ROOT/'frontend/js/ui-runtime.js').read_text();frontend_index=(ROOT/'frontend/index.html').read_text();app_vue=(ROOT/'frontend/components/App.vue').read_text()
    check('window.FVS_UI=Object.freeze' in ui_runtime and '...window.FVS_UI.methods' in app_vue,'pure frontend helpers are extracted from the SFC')
    check(frontend_index.index('/js/ui-runtime.js') < frontend_index.index('/js/bootstrap.js'),'UI runtime loads before SFC bootstrap')
    check('new Intl.NumberFormat' not in app_vue and 'data:image/svg+xml' not in app_vue,'App.vue no longer owns pure formatting/fallback payloads')
    check('experiment_snapshot' in marketing_py and 'experimentSnapshot' in marketing_php,'both marketing workers persist experiment evidence after metric sync')
    check('abandonment_scan' in marketing_py and 'abandonmentScan' in marketing_php,'both marketing workers run the guarded abandonment scanner')
    check('growth_loop_run' in marketing_py and 'growthLoopRun' in marketing_php and 'growth_decision_apply' in marketing_py and 'growthDecisionApply' in marketing_php,'both marketing workers scan and reconcile managed growth decisions')
    ...
    check('c.budget_minor=0' in marketing_runtime and 'c.daily_budget_minor=0' in marketing_runtime and 'c.max_daily_spend_minor=0' in marketing_runtime,'job claiming enforces lifetime and daily budget ceilings')
    check('GREATEST(COALESCE((SELECT SUM(ms.spend_minor)' in marketing_runtime and 'SUM(mr.revenue_minor)' in marketing_runtime,'stop-loss is enforced on net loss rather than gross spend')
    check("j.action<>'update_budget' OR c.target_roas_bps=0" in marketing_runtime and "closed_loop_growth" in marketing_runtime,'budget increases require target ROAS, with a scoped closed-loop ceiling bypass')
    check('authoritative_revenue_minor' in growth_loop and "FROM orders" in growth_loop and "min_spend_minor" in growth_loop,'growth loop uses authoritative orders and explicit configured evidence floors')
