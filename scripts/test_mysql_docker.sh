...
docker run -d --rm --name "$NAME" -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=fvs -e MYSQL_USER=fvs -e MYSQL_PASSWORD=fvs -p "$PORT:3306" mysql:8.4.12 --default-time-zone=+00:00 --transaction-isolation=READ-COMMITTED >/dev/null
