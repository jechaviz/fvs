...
final class FvsCore {
    ...
    public function inventorySearch(array $f):array{$bbox=$f['bbox']??null;$use=is_array($bbox)&&count($bbox)===4?1:0;if(!$use)$bbox=[0,0,0,0];$r=$this->jsonCall('fvs_inventory_search_v2',[(string)($f['q']??''),(string)($f['check_in']??''),(string)($f['check_out']??''),(int)($f['guests']??2),(string)($f['country']??''),(string)($f['city']??''),(string)($f['resort']??''),(string)($f['property_type']??''),(string)($f['season']??''),(string)($f['booking_mode']??''),(string)($f['amenities']??''),(int)($f['min_bedrooms']??0),(float)($f['min_bathrooms']??0),(int)($f['min_rating_x100']??0),(int)($f['min_price_minor']??0),(int)($f['max_price_minor']??0),(float)$bbox[0],(float)$bbox[1],(float)$bbox[2],(float)$bbox[3],$use,(string)($f['sort']??'relevance'),(int)($f['limit']??60),(int)($f['offset']??0)]);if((int)($f['offset']??0)===0)$this->commerceEventSafe(['event_type'=>'search','source'=>'server','query_text'=>substr((string)($f['q']??''),0,300),'result_count'=>(int)($r['total']??count($r['items']??[])),'metadata_json'=>['sort'=>(string)($f['sort']??'relevance'),'country'=>(string)($f['country']??''),'city'=>(string)($f['city']??''),'ranking_version'=>$this->searchRankingVersion()]]);return $r;}
    ...
    public function cartAdd(string $id,string $secret,string $slot,int $guests,int $hold):array{$r=$this->jsonCall('fvs_cart_add',[$id,$secret,$slot,$guests,$hold]);$this->commerceEventSafe(['event_key'=>"cart_add:{$id}:v".($r['version']??0).":{$slot}",'event_type'=>'cart_add','source'=>'server','cart_id'=>$id,'slot_id'=>$slot,'channel'=>$r['source_channel']??'','outcome'=>'held','value_minor'=>(int)($r['total_minor']??0),'currency'=>$r['currency']??'','metadata_json'=>['guests'=>$guests]]);return $r;}
    public function cartRemove(string $id,string $secret,string $slot):array{return $this->jsonCall('fvs_cart_remove',[$id,$secret,$slot]);}
    public function checkoutBegin(string $id,string $secret,string $provider,string $email,string $terms,int $hold):array{$r=$this->jsonCall('fvs_checkout_begin',[$id,$secret,$provider,$email,$terms,$hold]);$attempt=(string)($r['attempt_id']??'');$outcome=(($r['status']??'')==='quote_refreshed')?'requote':'started';$this->commerceEventSafe(['event_key'=>$attempt!==''?"checkout:{$attempt}":'','event_type'=>'checkout_start','source'=>'server','cart_id'=>$id,'attempt_id'=>$attempt,'provider'=>$provider,'outcome'=>$outcome,'reason_code'=>$outcome==='requote'?'quote_refreshed':'','value_minor'=>(int)($r['amount_minor']??$r['total_minor']??0),'currency'=>$r['currency']??'']);if($attempt!=='')$this->paymentRecoveryMark($attempt,'active','checkout_started','',0);return $r;}
    public function checkoutGet(string $id,string $secret):array{$r=$this->jsonCall('fvs_checkout_get',[$id,$secret]);$aid=(string)($r['attempt_id']??'');if($aid!==''){try{$r['recovery']=$this->paymentRecoveryGet($aid);}catch(CoreException $x){if($x->coreCode!==3)throw $x;}}return $r;}
    ...
    public function paymentConfirm(string $attempt,string $provider,string $payment,int $amount,string $currency,int $version):array{$r=$this->jsonCall('fvs_payment_confirm',[$attempt,$provider,$payment,$amount,$currency,$version]);$status=(string)($r['status']??'');if($status==='confirmed'&&!empty($r['order_id'])){$this->commerceEventSafe(['event_key'=>'booking:'.$r['order_id'],'event_type'=>'booking','source'=>'provider','attempt_id'=>$attempt,'order_id'=>$r['order_id'],'provider'=>$provider,'outcome'=>'confirmed','value_minor'=>(int)($r['total_minor']??$amount),'currency'=>$r['currency']??$currency]);$this->paymentRecoveryMark($attempt,'resolved','payment_confirmed','',0);}elseif($status==='manual_review'){$this->commerceEventSafe(['event_key'=>"payment:{$attempt}:manual_review",'event_type'=>'payment','source'=>'provider','attempt_id'=>$attempt,'provider'=>$provider,'outcome'=>'manual_review','reason_code'=>'reconciliation']);$this->paymentRecoveryMark($attempt,'manual_review','reconciliation','',0);}return $r;}
    ...
    public function growthLoopRun(int $days=7,int $limit=50):array{return $this->jsonCall('fvs_growth_loop_run',[$days,$limit]);}
    public function growthDecisionApply(string $jobId):void{$this->check($this->ffi->fvs_growth_decision_apply($this->ctx,$jobId));}
    public function growthLoopDashboard(int $days=30):array{return $this->jsonCall('fvs_growth_loop_dashboard',[$days]);}

    public function abandonmentScan(int $idleSeconds=1800,int $limit=50):array{return $this->jsonCall('fvs_abandonment_scan',[$idleSeconds,$limit]);}
    public function abandonmentDashboard(int $days=30):array{return $this->jsonCall('fvs_abandonment_dashboard',[$days]);}

    public function experimentSnapshot(string $campaignId,int $days=30):array{return $this->jsonCall('fvs_experiment_snapshot',[$campaignId,$days]);}
    public function experimentDashboard(int $days=30):array{return $this->jsonCall('fvs_experiment_dashboard',[$days]);}

    public function searchRankingVersion():string{$p=$this->ffi->fvs_search_ranking_version();return FFI::string($p);}
    public function searchEvalRecord(string $rankingVersion,string $corpusSha,float $mrr,float $ndcg10,float $precision10,int $queryCount):array{$ppm=static fn(float $v):int=>max(0,min(1000000,(int)round($v*1000000)));return $this->jsonCall('fvs_search_eval_record',[$rankingVersion,$corpusSha,$ppm($mrr),$ppm($ndcg10),$ppm($precision10),$queryCount]);}
    public function searchQualityDashboard(int $days=30):array{return $this->jsonCall('fvs_search_quality_dashboard',[$days]);}

    public function paymentRecoveryMark(string $attempt,string $state,string $reason='',string $error='',int $retryAfter=0):void{$this->check($this->ffi->fvs_payment_recovery_mark($this->ctx,$attempt,$state,$reason,$error,$retryAfter));}
    public function paymentRecoveryGet(string $attempt):array{return $this->jsonCall('fvs_payment_recovery_get',[$attempt]);}
    public function paymentRecoveryDashboard(int $days=30):array{return $this->jsonCall('fvs_payment_recovery_dashboard',[$days]);}

    public function commerceEventRecord(array $v):void{$meta=$v['metadata_json']??'';if(is_array($meta))$meta=json_encode($meta,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR);$this->check($this->ffi->fvs_commerce_event_record($this->ctx,(string)($v['event_key']??''),(string)$v['event_type'],(string)($v['source']??'server'),(string)($v['visitor_id']??''),(string)($v['session_id']??''),(string)($v['cart_id']??''),(string)($v['attempt_id']??''),(string)($v['order_id']??''),(string)($v['slot_id']??''),(string)($v['campaign_id']??''),(string)($v['creative_id']??''),(string)($v['channel']??''),(string)($v['provider']??''),(string)($v['outcome']??''),(string)($v['reason_code']??''),(int)($v['value_minor']??0),(string)($v['currency']??''),(string)($v['query_text']??''),(int)($v['result_count']??-1),(string)$meta));}
    private function commerceEventSafe(array $v):void{try{$this->commerceEventRecord($v);}catch(Throwable){}}
    public function commerceFunnelDashboard(int $days=30):array{return $this->jsonCall('fvs_commerce_funnel_dashboard',[$days]);}
    public function marketingDashboard(int $days=30):array{$r=$this->jsonCall('fvs_marketing_dashboard',[$days]);try{$r['funnel']=$this->commerceFunnelDashboard($days);}catch(Throwable){$r['funnel']=['unavailable'=>true];}try{$r['recovery']=$this->paymentRecoveryDashboard($days);}catch(Throwable){$r['recovery']=['unavailable'=>true];}try{$r['search_quality']=$this->searchQualityDashboard($days);}catch(Throwable){$r['search_quality']=['unavailable'=>true];}try{$r['experiments']=$this->experimentDashboard($days);}catch(Throwable){$r['experiments']=['unavailable'=>true];}try{$r['abandonment']=$this->abandonmentDashboard($days);}catch(Throwable){$r['abandonment']=['unavailable'=>true];}try{$r['growth_loop']=$this->growthLoopDashboard($days);}catch(Throwable){$r['growth_loop']=['unavailable'=>true];}return $r;}
