...
int fvs_payment_recovery_mark(fvs_ctx *ctx, const char *attempt_id, const char *state,
    ...
    if (!ctx || !fvs_i_valid_uuid(attempt_id) || !recovery_state_valid(state) ||
        ...
        retry_after_seconds > 86400U) {
        return FVS_ERR_INVALID;
    }

    char *es = fvs_i_escape(ctx, state);
    char *er = fvs_i_escape(ctx, reason_code ? reason_code : "");
    char *ee = fvs_i_escape(ctx, last_error ? last_error : "");
    if (!es || !er || !ee) {
        free(es);
        free(er);
        free(ee);
    ...
    const unsigned int initial_retries = strcmp(state, "provider_error") == 0 ? 1U : 0U;
    const int needed = snprintf(
        ...
        es, er, initial_retries, retry_after_seconds, retry_after_seconds, ee, es, attempt_id);
    if (needed < 0) {
        free(es);
        free(er);
        free(ee);
...
int fvs_payment_recovery_get(fvs_ctx *ctx, const char *attempt_id, char *out, size_t out_size) {
    ...
    int rc = fvs_i_queryf(
        ...
        "FROM payment_recovery_cases r JOIN payment_attempts p ON p.id=r.attempt_id WHERE r.attempt_id='%s'",
        attempt_id);
    ...
    if (!row) {
        ...
        return FVS_ERR_NOT_FOUND;
    }

    fvs_i_json w;
    fvs_i_json_init(&w, out, out_size);
    fvs_i_json_puts(&w, "{\"attempt_id\":");
    fvs_i_json_string(&w, attempt_id);
    fvs_i_json_puts(&w, ",\"state\":");
    fvs_i_json_string(&w, row[0]);
    fvs_i_json_puts(&w, ",\"reason_code\":");
    if (row[1] && *row[1]) fvs_i_json_string(&w, row[1]); else fvs_i_json_puts(&w, "null");
    fvs_i_json_printf(&w, ",\"retry_count\":%u,\"next_retry_at\":",
                      row[2] ? (unsigned int)strtoul(row[2], NULL, 10) : 0U);
    if (row[3]) fvs_i_json_string(&w, row[3]); else fvs_i_json_puts(&w, "null");
    fvs_i_json_puts(&w, ",\"last_error\":");
    if (row[4] && *row[4]) fvs_i_json_string(&w, row[4]); else fvs_i_json_puts(&w, "null");
    fvs_i_json_puts(&w, ",\"first_seen_at\":");
    fvs_i_json_string(&w, row[5]);
    fvs_i_json_puts(&w, ",\"last_seen_at\":");
    fvs_i_json_string(&w, row[6]);
    fvs_i_json_puts(&w, ",\"resolved_at\":");
    if (row[7]) fvs_i_json_string(&w, row[7]); else fvs_i_json_puts(&w, "null");
    fvs_i_json_puts(&w, ",\"provider\":");
    fvs_i_json_string(&w, row[8]);
    fvs_i_json_puts(&w, ",\"payment_status\":");
    fvs_i_json_string(&w, row[9]);
    fvs_i_json_puts(&w, ",\"cart_id\":");
    fvs_i_json_string(&w, row[10]);
    fvs_i_json_puts(&w, "}");
    mysql_free_result(res);
...
int fvs_payment_recovery_dashboard(fvs_ctx *ctx, unsigned int days, char *out, size_t out_size) {
    ...
    int rc = fvs_i_queryf(
        ...
        "FROM payment_recovery_cases WHERE first_seen_at>=TIMESTAMPADD(DAY,-%u,UTC_TIMESTAMP())",
        days);
    if (rc != FVS_OK) return rc;

    MYSQL_ROW row = mysql_fetch_row(res);
    const unsigned long long total = row && row[0] ? strtoull(row[0], NULL, 10) : 0ULL;
    const unsigned long long active = row && row[1] ? strtoull(row[1], NULL, 10) : 0ULL;
    const unsigned long long provider_error = row && row[2] ? strtoull(row[2], NULL, 10) : 0ULL;
    const unsigned long long manual = row && row[3] ? strtoull(row[3], NULL, 10) : 0ULL;
    const unsigned long long abandoned = row && row[4] ? strtoull(row[4], NULL, 10) : 0ULL;
    const unsigned long long resolved = row && row[5] ? strtoull(row[5], NULL, 10) : 0ULL;
    const unsigned long long stale = row && row[6] ? strtoull(row[6], NULL, 10) : 0ULL;
    mysql_free_result(res);
    ...
    fvs_i_json_printf(
        &w,
        "{\"days\":%u,\"total\":%llu,\"active\":%llu,\"provider_error\":%llu,\"manual_review\":%llu,"
        "\"abandoned\":%llu,\"resolved\":%llu,\"stale_recoverable\":%llu,\"by_provider\":[",
    ...
    rc = fvs_i_queryf(
        ...
        "WHERE r.first_seen_at>=TIMESTAMPADD(DAY,-%u,UTC_TIMESTAMP()) "
        "GROUP BY p.provider,r.state ORDER BY p.provider,r.state",
        days);
    if (rc != FVS_OK) return rc;
    ...
    while ((row = mysql_fetch_row(res)) != NULL) {
        ...
        fvs_i_json_printf(&w, ",\"count\":%llu}", row[2] ? strtoull(row[2], NULL, 10) : 0ULL);
    }
    mysql_free_result(res);
    fvs_i_json_puts(&w, "]}");
    return fvs_i_json_result(ctx, &w);
