...
int fvs_marketing_job_claim(fvs_ctx *ctx,const char *owner,unsigned int lease_seconds,
    ...
    rc=fvs_i_queryf(ctx,&res,
        ...
        "WHERE j.attempts<8 AND j.scheduled_at<=UTC_TIMESTAMP(6) "
        "AND ((j.status IN('pending','retry') AND j.next_attempt_at<=UTC_TIMESTAMP(6)) OR (j.status='processing' AND j.lease_until<=UTC_TIMESTAMP(6))) "
        "AND (j.lease_until IS NULL OR j.lease_until<=UTC_TIMESTAMP(6)) "
