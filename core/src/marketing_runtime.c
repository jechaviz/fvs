...
int fvs_marketing_job_claim(fvs_ctx *ctx,const char *owner,unsigned int lease_seconds,
    ...
    rc=fvs_i_queryf(ctx,&res,
        ...
        "AND ((j.status IN('pending','retry') AND j.next_attempt_at<=UTC_TIMESTAMP()) "
        "OR (j.status='processing' AND j.lease_until<=UTC_TIMESTAMP())) "
        "AND (j.lease_until IS NULL OR j.lease_until<=UTC_TIMESTAMP()) "
        "AND ("
          "j.action IN('pause','sync_metrics','send_conversion') "
          "OR (j.action='update_budget' "
              ...
              "AND (c.target_roas_bps=0 "
                   "OR COALESCE((SELECT SUM(gsp.spend_minor) FROM marketing_metrics_daily gsp WHERE gsp.campaign_id=c.id),0)=0 "
                   "OR ((CAST(COALESCE((SELECT SUM(grv.revenue_minor) FROM marketing_metrics_daily grv WHERE grv.campaign_id=c.id),0) AS DECIMAL(30,6))/"
                        "NULLIF(CAST(COALESCE((SELECT SUM(gsd.spend_minor) FROM marketing_metrics_daily gsd WHERE gsd.campaign_id=c.id),0) AS DECIMAL(30,6)),0))*10000)>=c.target_roas_bps)) "
          "OR (c.status IN('approved','scheduled','active') "
              ...
              "AND (j.action<>'update_budget' OR c.target_roas_bps=0 "
                   "OR COALESCE((SELECT SUM(mt.spend_minor) FROM marketing_metrics_daily mt WHERE mt.campaign_id=c.id),0)=0 "
                   "OR ((CAST(COALESCE((SELECT SUM(mv.revenue_minor) FROM marketing_metrics_daily mv WHERE mv.campaign_id=c.id),0) AS DECIMAL(30,6))/"
                        "NULLIF(CAST(COALESCE((SELECT SUM(mz.spend_minor) FROM marketing_metrics_daily mz WHERE mz.campaign_id=c.id),0) AS DECIMAL(30,6)),0))*10000)>=c.target_roas_bps))"
          ")"
