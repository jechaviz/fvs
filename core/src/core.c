...
int fvs_support_customer_message(fvs_ctx *ctx,const char *thread_id,const char *secret,const char *body,char *out,size_t out_size){
    ...
    char *sql = NULL;
    rc = sqlf(ctx,&sql,"INSERT INTO support_messages(id,thread_id,sender_type,body,visibility,created_at) VALUES('%s','%s','customer','%s','public',UTC_TIMESTAMP())",mid,thread_id,eb);
    if (rc == FVS_OK) rc = exec_sql(ctx,sql);
    free(sql); sql = NULL;
    if (rc == FVS_OK) {
        rc = sqlf(ctx,&sql,"UPDATE support_threads SET status=IF(status='waiting_customer','in_progress',status),last_message_at=UTC_TIMESTAMP(),updated_at=UTC_TIMESTAMP() WHERE id='%s'",thread_id);
        if (rc == FVS_OK) rc = exec_sql(ctx,sql);
        free(sql); sql = NULL;
    }
    if (rc == FVS_OK && ai) {
        rc = sqlf(ctx,&sql,"INSERT INTO outbox_events(id,event_type,aggregate_id,payload_json,status,attempts,next_attempt_at,created_at,updated_at) VALUES('%s','support.ai_requested','%s',JSON_OBJECT('thread_id','%s'),'pending',0,UTC_TIMESTAMP(),UTC_TIMESTAMP(),UTC_TIMESTAMP())",eid,thread_id,thread_id);
        if (rc == FVS_OK) rc = exec_sql(ctx,sql);
        free(sql);
    }
    free(eb);
...
int fvs_support_ai_message(fvs_ctx *ctx,const char *thread_id,const char *body,const char *model,unsigned int input_tokens,unsigned int output_tokens,char *out,size_t out_size){
    ...
    char *sql = NULL;
    rc = sqlf(ctx,&sql,"INSERT INTO support_messages(id,thread_id,sender_type,sender_id,body,visibility,model_name,input_tokens,output_tokens,created_at) VALUES('%s','%s','ai','support-ai','%s','public',%s%s%s,%u,%u,UTC_TIMESTAMP())",mid,thread_id,eb,*em?"'":"",*em?em:"NULL",*em?"'":"",input_tokens,output_tokens);
    if (rc == FVS_OK) rc = exec_sql(ctx,sql);
    free(sql); sql = NULL;
    if (rc == FVS_OK) {
        rc = sqlf(ctx,&sql,"UPDATE support_threads SET status='waiting_customer',last_message_at=UTC_TIMESTAMP(),updated_at=UTC_TIMESTAMP() WHERE id='%s'",thread_id);
        if (rc == FVS_OK) rc = exec_sql(ctx,sql);
        free(sql);
    }
    free(eb); free(em);
