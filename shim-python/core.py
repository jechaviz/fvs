...
lib.fvs_growth_loop_run.argtypes=[ctx_p,ctypes.c_uint,ctypes.c_uint,ctypes.c_char_p,size_t];lib.fvs_growth_loop_run.restype=ctypes.c_int
lib.fvs_growth_decision_apply.argtypes=[ctx_p,ctypes.c_char_p];lib.fvs_growth_decision_apply.restype=ctypes.c_int
lib.fvs_growth_loop_dashboard.argtypes=[ctx_p,ctypes.c_uint,ctypes.c_char_p,size_t];lib.fvs_growth_loop_dashboard.restype=ctypes.c_int
...
def marketing_attribution_record(**v):
    ...
    if rc:_err(c,rc)
def growth_loop_run(days=7,limit=50): return _json_call(lib.fvs_growth_loop_run,int(days),int(limit))
def growth_decision_apply(job_id):
    c=ctx();rc=lib.fvs_growth_decision_apply(c,b(job_id))
    if rc:_err(c,rc)
def growth_loop_dashboard(days=30): return _json_call(lib.fvs_growth_loop_dashboard,int(days))
...
def marketing_dashboard(days=30):
    ...
    try: result["growth_loop"]=growth_loop_dashboard(days)
    except Exception: result["growth_loop"]={"unavailable":True}
    return result
